...
public:
    ...
    struct RegTestOptions {
        ...
        bool digishield_strict{false}; // Doichain: disallow min-difficulty blocks so the headers-sync bound is enforced (regtest-only)
        std::optional<int64_t> pow_target_spacing{}; // Doichain: block target in seconds, to compress DAA experiments (regtest-only)
    };
