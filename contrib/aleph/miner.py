...
def wait_for_node(rpc, log):
    for i in range(600):
        try:
            rpc("getblockcount")
            return
        except Exception as e:                       # node still starting
            if i % 10 == 0:
...
def main():
    ...
    if args.hashrate > 0 and not address:
        try:
            ...
            except Exception:                         # already loaded
                pass
    ...
    while True:
        ...
        if found or height != last_height:
            note = f" FOUND {found[:16]}…" if found else ""
            log(f"height={height} bits={bits} difficulty={diff:.6g}{note}")
            last_height = height
        if csv:
        ...
        if delay > 0:
            time.sleep(delay)
        else:                                          # hashing took longer than a tick: re-anchor
            next_tick = time.monotonic()
